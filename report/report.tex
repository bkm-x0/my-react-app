...
Figure~\ref{fig:activity} presents the activity diagram illustrating the complete customer purchase flow, from browsing products through checkout to order confirmation.
...
\begin{tikzpicture}[
  ...
  finish/.style={circle, draw=black, line width=1.5pt, minimum size=8mm, path picture={\fill[black] (path picture bounding box.center) circle(2.5mm);}},
  action/.style={draw, rounded corners=6pt, minimum width=3.2cm, minimum height=0.7cm, fill=orange!10, font=\small, align=center},
  decision/.style={draw, diamond, aspect=2.5, fill=yellow!10, font=\scriptsize, align=center, inner sep=2pt},
  arr/.style={->, thick, >=stealth},
]
  \node[start] (s) at (0,0) {};
  \node[action] (browse) at (0,-1.5) {Browse Products};
  \node[decision] (found) at (0,-3.5) {Product found?};
  \node[action] (search) at (5.5,-3.5) {Search / Filter};
  \node[action] (view) at (0,-5.5) {View Product Details};
  \node[decision] (wantadd) at (0,-7.5) {Add to Cart?};
  \node[action] (addcart) at (0,-9.5) {Add to Cart};
  \node[decision] (cont) at (0,-11.5) {Continue shopping?};
  \node[action] (checkout) at (0,-13.5) {Go to Checkout};
  \node[decision] (auth) at (0,-15.5) {Logged in?};
  \node[action] (login) at (5.5,-15.5) {Login / Register};
  \node[action] (ship) at (0,-17.5) {Enter Shipping \& Payment};
  \node[action] (place) at (0,-19.5) {Confirm \& Place Order};
  \node[action] (done) at (0,-21.5) {Order Confirmed};
  \node[finish] (e) at (0,-23) {};

  \draw[arr] (s) -- (browse);
  \draw[arr] (browse) -- (found);
  \draw[arr] (found) -- node[right, font=\scriptsize] {Yes} (view);
  \draw[arr] (found) -- node[above, font=\scriptsize] {No} (search);
  \draw[arr] (search) |- (browse);
  \draw[arr] (view) -- (wantadd);
  \draw[arr] (wantadd) -- node[right, font=\scriptsize] {Yes} (addcart);
  \draw[arr] (wantadd.west) -- node[near start, above, font=\scriptsize] {No} ++(-2.5,0) |- (browse.west);
  \draw[arr] (addcart) -- (cont);
  \draw[arr] (cont.west) -- node[near start, above, font=\scriptsize] {Yes} ++(-3.5,0) |- (browse.west);
  \draw[arr] (cont) -- node[right, font=\scriptsize] {No} (checkout);
  \draw[arr] (checkout) -- (auth);
  \draw[arr] (auth) -- node[right, font=\scriptsize] {Yes} (ship);
...
\caption{Activity Diagram -- Customer Purchase Flow}
\label{fig:activity}
\end{figure}


\section{State Diagram -- Order Lifecycle}
Figure~\ref{fig:state-order} shows the state transitions of an order throughout its lifecycle. An order begins in the \textbf{Pending} state upon creation and can be confirmed, rejected, or cancelled. Confirmed orders proceed through processing, shipping, and delivery.

\begin{figure}[H]
\centering
\begin{tikzpicture}[
  state/.style={draw, rounded corners=8pt, minimum width=2.4cm, minimum height=0.8cm, fill=orange!10, font=\small\bfseries, align=center},
  start/.style={circle, fill=black, minimum size=5mm},
  finish/.style={circle, draw=black, line width=1.5pt, minimum size=7mm, path picture={\fill[black] (path picture bounding box.center) circle(2mm);}},
  arr/.style={->, thick, >=stealth},
  lbl/.style={font=\scriptsize, fill=white, inner sep=1pt},
]
  \node[start] (s) at (-3,0) {};
  \node[state] (pending) at (0,0) {Pending};
  \node[state] (confirmed) at (4,0) {Confirmed};
  \node[state] (processing) at (8,0) {Processing};
  \node[state] (shipped) at (12,0) {Shipped};
  \node[state] (delivered) at (12,-2.8) {Delivered};
  \node[state, fill=red!10] (rejected) at (4,-2.8) {Rejected};
  \node[state, fill=red!10] (cancelled) at (0,-2.8) {Cancelled};
  \node[finish] (e1) at (12,-4.5) {};
  \node[finish] (e2) at (4,-4.5) {};
  \node[finish] (e3) at (0,-4.5) {};

  \draw[arr] (s) -- (pending);
  \draw[arr] (pending) -- node[lbl, above] {Admin confirms} (confirmed);
  \draw[arr] (confirmed) -- node[lbl, above] {Start processing} (processing);
  \draw[arr] (processing) -- node[lbl, above] {Ship order} (shipped);
  \draw[arr] (shipped) -- node[lbl, right] {Delivered} (delivered);
  \draw[arr] (pending) -- node[lbl, right] {Admin rejects} (rejected);
  \draw[arr] (pending) -- node[lbl, right] {User cancels} (cancelled);
  \draw[arr] (delivered) -- (e1);
  \draw[arr] (rejected) -- (e2);
  \draw[arr] (cancelled) -- (e3);
\end{tikzpicture}
\caption{State Diagram -- Order Lifecycle}
\label{fig:state-order}
\end{figure}
